...
#define AWS_IOT_MQTT_PORT              12 ///< default port for MQTT/S
...
#define AWS_IOT_MQTT_RX_BUF_LEN 5120 ///< Any message that comes into the device should be less than this buffer size. If a received message is bigger than this buffer size the message will be dropped.
...
#define MAX_JSON_TOKEN_EXPECTED 50 ///< These are the max tokens that is expected to be in the Shadow JSON document. Include the metadata that gets published
