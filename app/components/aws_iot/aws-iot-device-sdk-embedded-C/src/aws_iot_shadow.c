...
const ShadowConnectParameters_t ShadowConnectParametersDefault = {(char *) AWS_IOT_MY_THING_NAME, (char *) "toybox",
...
IoT_Error_t aws_iot_shadow_connect(AWS_IoT_Client *pClient, const ShadowConnectParameters_t *pParams) {
	...
	ConnectParams.keepAliveIntervalInSec = 60; // NOTE: Temporary fix
	...
	if(NULL != pParams->deleteActionHandler) {
		snprintf(deleteAcceptedTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES,
				 "$aws/things/%s/shadow/name/%s/delete/accepted", myThingName, pParams->pMyShadowName);
...
IoT_Error_t aws_iot_shadow_register_delta(AWS_IoT_Client *pMqttClient, const char *pShadowName, jsonStruct_t *pStruct) {
	...
	return registerJsonTokenOnDelta(pStruct, pShadowName);
}
